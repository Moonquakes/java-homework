    ...
    public BattleField embattle(Creature[] creatures) {
        ...
        if (row < 7 || column < 4) {
            ...
            for(int i=0;i<3;i++){
                ...
                newRoles[i].setPosition(new Position(vacancyRow+2+i,vacancyColumn+11));
                newRoles[3+i].setPosition(new Position(vacancyRow+2+i,vacancyColumn+1+11));
            }
            for(int i=0;i<5;i++){
                field.battlefield[vacancyRow+1+i][vacancyColumn+2].setCreature(newRoles[6+i]);
                newRoles[6+i].setPosition(new Position(vacancyRow+1+i,vacancyColumn+2+11));
            }
            for(int i=0;i<7;i++){
                field.battlefield[vacancyRow+i][vacancyColumn+3].setCreature(newRoles[11+i]);
                newRoles[11+i].setPosition(new Position(vacancyRow+i,vacancyColumn+3+11));
            }
        }
        return field;
    }
